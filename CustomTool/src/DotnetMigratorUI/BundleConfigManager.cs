...
namespace DotnetMigratorUI
{
    public static class BundleConfigManager
    {
        ...
        private static List<BundleConfig> ProcessBundleConfigContent(string BundleConfigClassContent, string bundleName)
        {
            ...
            var scriptBundles = BundleConfigClassContent.Split(new string[] { bundleName },StringSplitOptions.RemoveEmptyEntries);
            int count = 0;
            foreach (var scriptBundle in scriptBundles)
            {
                if (count !=0 && scriptBundle?.Contains("Include") == true)
                {
                    var scriptBundleContent = scriptBundle.Split(new string[] { "Include" }, StringSplitOptions.RemoveEmptyEntries);
